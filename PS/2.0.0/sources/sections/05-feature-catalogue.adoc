...
used in the product. The {{PROD.id}} Feature Catalogue is available in an
...
*SpatialQuality*. Only points and curves can be associated
...
{{PROD.id}} defines attributes as either simple or complex.

[[sec_5.5.1]]
==== Simple attributes

{{PROD.id}} uses seven types of simple attributes; they are listed in the
following Table:
...
Calendar. +
In XML formats including {{PROD.id}}, the XML Schema standard type
...
present. Since {{PROD.id}} uses XML formats for both dataset and metadata,
the XML encoding of truncated dates must be used (i.e., the ISO 8601
basic format is not used in {{PROD.id}}).
...
| Time | A 24-hour time, it may contain a time zone. Since {{PROD.id}} uses
XML formats for both datasets and metadata, the XML encoding must
be used.

In XML formats (including {{PROD.id}}), the XML Schema standard
...
bindings. Bindings of complex attributes are represented in the S-122
UML diagrams by a local attribute (<<fig_5.1>>).

[[fig_5.1]]
.Complex attribute
image::ExampleComplexAttribute.png[]

[[sec_5.6]]
=== Units of measure

The following units of measure are used in {{PROD.name}} datasets;
